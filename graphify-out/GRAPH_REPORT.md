...
## Corpus Check
- 470 files · ~230,743 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1894 nodes · 2718 edges · 53 communities detected
...
Cohesion: 0.03
Nodes (58): createMailer(), isConfigured(), bufferIndexOf(), createBackupRouter(), parseMultipart(), splitBuffer(), deepMerge(), defaultConfigPath() (+50 more)
...
Cohesion: 0.03
Nodes (22): MetaDispatcher, NoPublisherError, OutboxDispatch, errorMessage(), OutboxPoller, canTransition(), IllegalTransitionError, OutboxNotFoundError (+14 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (19): PrivacyController, SessionManager, estimateTokens(), SmartRouter, CopilotWrapper, createModelRouter(), listen(), mount() (+11 more)
...
Cohesion: 0.07
Nodes (5): FakeDmSender, Metrics, zero(), createSocketServer(), TranscriptManager
...
- **Thin community `Community 40`** (8 nodes): `MetaScheduler`, `.constructor()`, `.schedule()`, `.start()`, `.stop()`, `.tick()`, `scheduler.test.ts`, `scheduler.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (8 nodes): `FacebookPages`, `.constructor()`, `.createPost()`, `.getInsights()`, `.listComments()`, `.listPages()`, `.reply()`, `.run()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (7 nodes): `TourPage`, `.coachMark()`, `.constructor()`, `.dismiss()`, `.dismissButton()`, `.goto()`, `tour.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (7 nodes): `DashboardPage`, `.constructor()`, `.dialog()`, `.goto()`, `.kpiCard()`, `.openQuickActions()`, `dashboard.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (6 nodes): `ComposePage`, `.constructor()`, `.goto()`, `.stubConnections()`, `.target()`, `compose.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (6 nodes): `InboxPage`, `.constructor()`, `.conversationList()`, `.goto()`, `.listOrEmptyState()`, `inbox.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
