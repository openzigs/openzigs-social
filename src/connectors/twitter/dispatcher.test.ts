...
import { TwitterApiError } from "./rest-client.js";
...
describe("TwitterDispatcher", () => {
  it("acquires a slot then runs the op on success", async () => {
    const broker = fakeBroker([{ granted: true }]);
    const { repo, landed } = fakeDlq();
    const dispatcher = new TwitterDispatcher({ broker, dlq: repo, retryOptions: noSleep });

    const run = vi.fn(async () => ({ id: "ok" }));
    const outcome = await dispatcher.dispatch(
      { platform: "twitter", opKind: "twitter.publish", payload: { a: 1 } },
      run
    );

    expect(outcome).toEqual({ ok: true, value: { id: "ok" }, attempts: 1 });
    expect(broker.acquire).toHaveBeenCalledWith("twitter", { cost: 1, timeoutMs: 30_000 });
  ...
  it("lands in the DLQ without running when the slot is denied", async () => {
    ...
    const { repo, landed } = fakeDlq();
    const dispatcher = new TwitterDispatcher({ broker, dlq: repo });

    const run = vi.fn(async () => "never");
    const outcome = await dispatcher.dispatch(
      { platform: "twitter-dm", opKind: "twitter.dm.send", payload: { x: 1 } },
      run
    );
  ...
  it("retries transient X errors then succeeds", async () => {
    const broker = fakeBroker([{ granted: true }]);
    const { repo, landed } = fakeDlq();
    const dispatcher = new TwitterDispatcher({ broker, dlq: repo, retryOptions: noSleep });
    ...
    const run = vi.fn(async () => {
      calls += 1;
      if (calls < 3) {
        throw new TwitterApiError("rate limited", { httpStatus: 429, transient: true });
      }
      return "done";
    });

    const outcome = await dispatcher.dispatch(
      { platform: "twitter", opKind: "twitter.publish", payload: {} },
      run
    );
  ...
  it("lands a terminal (non-transient) error in the DLQ", async () => {
    ...
    const run = vi.fn(async () => {
      throw new TwitterApiError("bad token", { httpStatus: 401, transient: false });
    });

    const outcome = await dispatcher.dispatch(
      { platform: "twitter", opKind: "twitter.publish", payload: { foo: "bar" } },
      run
    );

    expect(outcome.ok).toBe(false);
    expect(run).toHaveBeenCalledTimes(1);
    expect(landed[0]).toMatchObject({ platform: "twitter", opKind: "twitter.publish" });
  });
});
