...
import { TikTokApiError } from "./rest-client.js";
...
describe("TikTokDispatcher", () => {
  it("acquires then runs on success", async () => {
    ...
    const { repo } = fakeDlq();
    const dispatcher = new TikTokDispatcher({ broker, dlq: repo, retryOptions: noSleep });

    const outcome = await dispatcher.dispatch(
      { platform: "tiktok", opKind: "tiktok.publish", payload: {} },
      async () => ({ publish_id: "p" })
    );

    expect(outcome).toEqual({ ok: true, value: { publish_id: "p" }, attempts: 1 });
    expect(broker.acquire).toHaveBeenCalledWith("tiktok", { cost: 1, timeoutMs: 30_000 });
  });

  it("lands denied acquisitions in the DLQ without running", async () => {
    const broker = fakeBroker([{ granted: false, reason: "timeout" }]);
    const { repo, landed } = fakeDlq();
    const dispatcher = new TikTokDispatcher({ broker, dlq: repo });
  ...
  it("retries transient errors then succeeds", async () => {
    ...
    let calls = 0;
    const outcome = await dispatcher.dispatch(
      { platform: "tiktok", opKind: "x", payload: {} },
      async () => {
        calls += 1;
        if (calls < 2) throw new TikTokApiError("rl", { httpStatus: 429, transient: true });
        return "done";
      }
    );
  ...
  it("lands terminal errors in the DLQ", async () => {
    ...
    const outcome = await dispatcher.dispatch(
      { platform: "tiktok", opKind: "x", payload: {} },
      async () => {
        throw new TikTokApiError("bad", { httpStatus: 401, transient: false });
      }
    );
